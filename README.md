...
| `style_block_size` | 80 | `stylize_model` の粒度。小さくすると 1 ブロックが細かい |
...
| `voxel_lower_bound` | **6** | 区画が小さくても 1 voxel ≒ 1 Minecraft ブロックを保証する下限（デフォルト） |
| `voxel_upper_bound` | 48 | 大規模建築でもブロックが大きくなり過ぎないようクランプ |
| `model_seed` / `texture_seed` | 42 | 固定すると同じ画像で同じ 3D が再現される |
...
#### `voxel_lower_bound` のチューニング指針

- ボクセル解像度は `target_voxel = max(lower, min(upper, max(width, depth)))` でクランプされます。
- City Plan の区画が 10×10 程度なら `voxel_lower_bound = 6`（デフォルト）または `12` で **concept art の 1 ブロック ≒ Minecraft の 1 ブロック**に近づきます。Pixel Art なら `lo=4, hi=5`（Settings の Voxel 解像度）。
- 「1 ブロックが 10 倍に拡大されたドット絵になる」 → `voxel_lower_bound` を下げる。
- 「建物がカクカクで細部が消える」 → `voxel_lower_bound` を 16〜20 に上げる、もしくは `voxel_upper_bound` を増やす。
- 旧挙動（解像度高め）に戻したい場合は `voxel_lower_bound = 28`、`voxel_upper_bound = 96` を設定してください。

Building ページの PipelineStatus には `style` / `target_voxel` / `face_limit` などが表示されるので、何が効いているか確認しながら調整できます。
